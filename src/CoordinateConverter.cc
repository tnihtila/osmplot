...
std::string CoordinateConverter::zoneLetters_m("CDEFGHJKLMNPQRSTUVW");
...
std::tuple<double, double, unsigned int, char>
  CoordinateConverter::LonLatToUTM(double lon_deg, double lat_deg)
{
    ...
    unsigned int i = 0;
    for (double d = firstZone_deg_m; d <= lastZone_deg_m;
         d += zoneStep_deg_m, ++i)
    {
        if (lat_deg < d)
        {
    ...
    double v =
      (c_m / sqrt((1 + (e2squared_m * pow(cos(lat_rad), 2))))) * 0.9996;
    ...
    double Bm =
      0.9996 * c_m * (lat_rad - alpha_m * j2 + beta_m * j4 - gamma_m * j6);
    ...
    return std::make_tuple(x, y, UTMZone, UTMZoneLetter);
}
                                              ...
                                              double lat2_deg)
{
    ...
    double a = sin(dLat / 2) * sin(dLat / 2) + cos(lat1_deg * M_PI / 180.0) *
                                                 cos(lat2_deg * M_PI / 180.0) *
