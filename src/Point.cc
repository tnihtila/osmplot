...
std::tuple<double, double, double, double>
  Point::worldLimits_m(std::numeric_limits<double>::max(),
...
Point::Point() : data_m(0, 0) {}

Point::Point(double x, double y) : data_m(x, y) {}

Point::Point(std::pair<double, double> data) : data_m(data) {}

Point::~Point() {}

Point::Point(const Point& source) : data_m(source.data_m) {}
...
void Point::Convert()
{
    ...
    {
        auto minTmp =
          conv_m.LonLatToUTM(get<0>(worldLimits_m), get<1>(worldLimits_m));
        get<0>(worldLimits_m) = get<0>(minTmp);
        get<1>(worldLimits_m) = get<1>(minTmp);
        auto maxTmp =
          conv_m.LonLatToUTM(get<2>(worldLimits_m), get<3>(worldLimits_m));
...
double Point::x() const { return data_m.first; }

double Point::y() const { return data_m.second; }
...
Point Point::ProjectionToLine(const Line& line) const
{
    ...
    double l2 = line.a().DistSqr(
      line.b()); // length_squared(v, w);  // i.e. |w-v|^2 -  avoid a sqrt
    ...
    double t =
      ((data_m.first - line.a().x()) * (line.b().x() - line.a().x()) +
       (data_m.second - line.a().y()) * (line.b().y() - line.a().y())) /
      l2;
...
int Point::Flip_Y(int y, int height) { return height - y; }
